...
{ NFE.procNFe.digVal  := LerCampo(Grupo, 'Digest Value da NF-e');
 ...
 NFe.Emit.EnderEmit.cMun    := StrToIntDef(LerCampo(Grupo,'Município',7),7);
 ...
 NFe.Emit.EnderEmit.UF      := LerCampo(Grupo,'UF');
 NFe.Emit.IE                := OnlyAlphaNum(LerCampo(Grupo,'Inscrição Estadual'));
 Nfe.Emit.IEST              := OnlyAlphaNum(LerCampo(Grupo,'Inscrição Estadual do Substituto Tributário'));
 ...
 NFe.Dest.EnderDest.cMun    := StrToIntDef(LerCampo(Grupo,'Município',7),7);
     ...
     begin
       ...
       with Imposto.ICMS do
        begin
          ...
          //Modalidade Definição da BC ICMS SUBSTITUTO
...
// Grupo :=  SeparaAte('Dados de Cobrança',ArquivoRestante,ArquivoItens);
 ...
 try
    ...
    GeradorXML.GerarXml;
    CaminhoXML        := PathWithDelim(ExtractFilePath('C:\siscom\SERVER\nfe\xml_entradas\'))+copy(NFe.infNFe.ID, (length(NFe.infNFe.ID)-44)+1, 44)+'-nfe.xml';
