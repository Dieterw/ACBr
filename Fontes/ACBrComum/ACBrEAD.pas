...
begin
  ...
  ChaveRSA := '' ;
  if Assigned( fsOnGetChavePrivada ) then
    fsOnGetChavePrivada( ChaveRSA ) ;

  if ChaveRSA = '' then
    raise Exception.Create( ACBrStr('Chave RSA não especificada no evento: "OnGetChavePrivada"') ) ;
...
//     fsOnCalcEAD( NomeArquivo ) /// Se usuário usa outro método para assinar o arquivo
//  else
//     WriteRegistroEAD( NomeArquivo );
