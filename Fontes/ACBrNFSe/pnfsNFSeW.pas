...
type
 ...
 TNFSeW = class(TPersistent)
  private
    ...
    FAtributo: String;
    FPrefixo4: String;
    FCodigoSchema: String;
    FPathSchema: String;
    FIdentificador: String;

    procedure GerarIdentificacaoRPS;
    procedure GerarRPSSubstituido;
    procedure GerarServico(AVersao:String);
    procedure GerarPrestador(AVersao:String);
    procedure GerarTomador(AVersao:String);
  ...
  published
    ...
    property CodigoSchema: String read FCodigoSchema write FCodigoSchema;
    property PathSchema: String read FPathSchema write FPathSchema;
...
begin
 ...
 IniFile := PathWithDelim(FPathSchema) + 'Config_Schemas.ini';

 Ini := TIniFile.Create( IniFile );
 try
  URL           := ini.ReadString(FCodigoSchema, 'URL',       '');
  VersaoXML     := ini.ReadString(FCodigoSchema, 'VersaoXML', '');
  DefTipos      := ini.ReadString(FCodigoSchema, 'DefTipos',  '');
  ServicoEnviar := ini.ReadString(FCodigoSchema, 'ServicoEnviar',  '');
 finally
  Ini.Free;
 end;
 ...
 if FProvedor = 'Prodemge'
  then DefTipos := ServicoEnviar;
  
 if Trim(Prefixo4) <> ''
  then Atributo := ' xmlns:' + stringReplace(Prefixo4, ':', '', []) + '="' + URL + DefTipos + '"'
  else Atributo := ' xmlns="' + URL + DefTipos + '"';
{
 if CTe.procCTe.nProt <> '' then
  Gerador.wGrupo('NfseProc');
}
 ...
 GerarRPSSubstituido;
 GerarServico(VersaoXML);
 GerarPrestador(VersaoXML);
 GerarTomador(VersaoXML);
...
procedure TNFSeW.GerarServico(AVersao:String);
var
  i: integer;
begin
 ...
 Gerador.wCampoNFSe(tcStr, '#20', 'IssRetido             ', 01, 01, 1, SimNaoToStr(NFSe.Servico.Valores.IssRetido), '');
 // No provedor Ginfes o ValorIss é obrigatório
 ...
 if FProvedor <> 'GovBR'
  then begin
   ...
   // No provedor Ginfes a BaseCalculo é obrigatório
...
// Gerador.wCampoNFSe(tcStr, '#29', 'ItemListaServico         ', 01, 0005, 1, SomenteNumeros(NFSe.Servico.ItemListaServico), '');
...
procedure TNFSeW.GerarPrestador(AVersao:String);
begin
...
procedure TNFSeW.GerarTomador(AVersao:String);
begin
 ...
 if AVersao='1'
  then begin
   ...
   Gerador.wCampoNFSe(tcStr, '#43', 'CodigoMunicipio', 007, 007, 0, SomenteNumeros(NFSe.Tomador.Endereco.CodigoMunicipio), '');
