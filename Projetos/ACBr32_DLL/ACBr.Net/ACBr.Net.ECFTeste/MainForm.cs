...
namespace ACBr.Net.ECFTeste
{
    public partial class MainForm : Form
    {
        ...
        const string NUM_FAB = "NUMFAB78901234567890";
        const string MF_ADICIONAL = "";
        const string TIPO_ECF = "ECF-IF";
        const string MARCA_ECF = "ACBr";
        const string MODELO_ECF = "PAF";       
        ...
        private string GerarDados(char tipo, int size)
        {
            ...
            if(tipo == 'S')
                StrCaracteres = "abcdefghijklmnopqrstuvwxyz";
            else if(tipo == 'I')
                StrCaracteres = "0123456789";
            else
                return string.Empty;

            int valormaximo = StrCaracteres.Length - 1;
            Random random = new Random(DateTime.Now.Millisecond);
            StringBuilder str = new StringBuilder(size);

            for (int ind = 0; ind < size; ind++)
                str.Append(StrCaracteres[random.Next(0, valormaximo)]);
